...
internals.fetchTokenFromApi = function() {
  ...
  request(options, function(err, response, body) {
    if (err) {
      ...
      self.cache.setToken(token).then(function() { deferred.resolve(token); });
    }
  });
  ...
  self.getToken().then(function(token) {
    ...
    request(options, function(err, response, body) {
      if (err) {
        ...
        var filteredPhotoData = photoData.filter(function(element, index, array) {
          return element.PictureIdentification !== null;
        });
        internals.resizeImagesToWidth.call(self, filteredPhotoData).then(function(photoData) {
...
internals.resizeImagesToWidth = function(data, width) {
  ...
  if (!self.config.hasOwnProperty('maxWidth')) {
    ...
    data.forEach(function(item) {
      if (!item.PictureIdentification) { return; }
