...
function PhotoClient(options) {
  ...
  } else if ((typeof config.cache.store === 'string') && (fs.existsSync(path.resolve(__dirname, '..', 'cache_stores', (config.cache.store + '.js'))))) {
...
PhotoClient.prototype.getToken = function(cb) {
  ...
  self.cache.getToken().then(function(token) {
    if (!token) {
...
PhotoClient.prototype.getPhoto = function(ids, cb) {
  ...
  self.cache.getPhotos(ids).then(function(results) {
